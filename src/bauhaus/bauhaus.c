...
dt_bauhaus_popup_motion_notify(GtkWidget *widget, GdkEventMotion *event, gpointer user_data)
{
  ...
  // allocation widget arg
  GtkAllocation allocation_widget;
  gtk_widget_get_allocation(widget, &allocation_widget);

  // allocation current widget
  GtkWidget *cw = GTK_WIDGET(darktable.bauhaus->current);
  GtkAllocation allocation_current;
  gtk_widget_get_allocation(cw, &allocation_current);

  // allocation popup widget
  GtkWidget *pw = GTK_WIDGET(darktable.bauhaus->popup_window);
  GtkAllocation allocation_popup;
  gtk_widget_get_allocation(pw, &allocation_popup);

  int width = allocation_popup.width, height = allocation_popup.height;
  switch(w->type)
  {
    case DT_BAUHAUS_COMBOBOX:
    ...
    case DT_BAUHAUS_SLIDER:
    {
      ...
      const float mouse_off = get_slider_line_offset(
                                ...
                                event->y/height, allocation_current.height/(float)height,
...
{
  GtkAllocation allocation;
  gtk_widget_get_allocation(GTK_WIDGET(darktable.bauhaus->current), &allocation);
...
dt_bauhaus_widget_accept(dt_bauhaus_widget_t *w)
{
  ...
  GtkAllocation allocation_popup;
  gtk_widget_get_allocation(GTK_WIDGET(darktable.bauhaus->popup_window), &allocation_popup);
  ...
  switch(w->type)
  {
    case DT_BAUHAUS_COMBOBOX:
    {
      ...
      dt_bauhaus_combobox_data_t *d = &w->data.combobox;
      int active = darktable.bauhaus->end_mouse_y / (allocation_widget.height + get_line_space());
      GList *it = d->labels;
      int k = 0, i = 0, kk = 0;
      while(it)
      {
    ...
    case DT_BAUHAUS_SLIDER:
    {
      ...
      const float mouse_off = get_slider_line_offset(
                                ...
                                darktable.bauhaus->end_mouse_y/height, allocation_widget.height/(float)height,
                                allocation_widget.width);
      dt_bauhaus_slider_set_normalized(w, d->oldpos + mouse_off);
      d->oldpos = d->pos;
      break;
...
dt_bauhaus_popup_expose(GtkWidget *widget, GdkEventExpose *event, gpointer user_data)
{
  ...
  GtkAllocation allocation_widget;
  gtk_widget_get_allocation(widget, &allocation_widget);
  int width = allocation_widget.width, height = allocation_widget.height;
...
dt_bauhaus_show_popup(dt_bauhaus_widget_t *w)
{
  ...
  switch(darktable.bauhaus->current->type)
  {
    ...
    case DT_BAUHAUS_COMBOBOX:
    {
      ...
      dt_bauhaus_combobox_data_t *d = &w->data.combobox;
      gtk_widget_set_size_request(darktable.bauhaus->popup_area, allocation.width, (allocation.height+get_line_space()) * d->num_labels);
      const int ht = allocation.height;
      const int skip = ht + get_line_space();
...
dt_bauhaus_slider_button_press(GtkWidget *widget, GdkEventButton *event, gpointer user_data)
{
  ...
  else if(event->button == 1)
  {
    ...
    {
      const float l = 4.0f/allocation.width;
      const float r = 1.0f-(allocation.height+4.0f)/allocation.width;
